...
-- * Generation date: Thu Jun 30 08:46:25 2011 
-- * LUN file: C:\Users\benoit.toussaint\Documents\My Dropbox\ProjetsPerso\eXa\Extended Agenda For Work Hours Control.lun 
...
create table Jour_Projet (
     ...
     ID numeric(10) not null,
     constraint ID_Jour_Projet primary key (ID, J_J_ID),
...
create table Lieu_Jour (
     ...
     ID numeric(1) not null,
     constraint ID_Lieu_Jour primary key (L_J_ID, ID),
...
create table Type_Journee (
     ...
     constraint ID_Type_Journee primary key (T_J_ID, ID),
...
create unique index ID_Jour_Projet
     on Jour_Projet (ID, J_J_ID);

create index FKJou_Jou
     on Jour_Projet (J_J_ID);

create unique index ID_Lieu_Jour
     on Lieu_Jour (L_J_ID, ID);

create index FKLie_Lie
     on Lieu_Jour (ID);

create unique index ID_Type_Journee
     on Type_Journee (T_J_ID, ID);

create index FKTyp_Typ
     on Type_Journee (ID);
